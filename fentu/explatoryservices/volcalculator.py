...
def plot_extended_tail_distribution(ticker, confidence_levels=[0.01, 0.001, 0.0001]):
    """
    Plot the extended tail distribution to visualize potential black swan events
    beyond what's observed in historical data.
    
    Args:
        ticker: The financial instrument ticker
        confidence_levels: List of probability thresholds for extreme events
                          (default: 1%, 0.1%, and 0.01% probability events)
    """
    ...
    # Fit t-distribution to negative returns
    from scipy.stats import t
    params = t.fit(negative_returns)
    df, loc, scale = params
    
    # Find the historical minimum
    historical_min = monthly_returns.min()
    
    # Calculate theoretical extremes at different confidence levels
    theoretical_extremes = [t.ppf(level, df, loc, scale) for level in confidence_levels]
    
    # Create the plot
    fig, ax = plt.subplots(figsize=(12, 8))
    
    # Plot historical data histogram
    ax.hist(negative_returns, bins=30, density=True, alpha=0.6, color='steelblue', 
            label='Historical Negative Returns')
    
    # Plot the fitted distribution
    x_range = np.linspace(min(historical_min * 2, theoretical_extremes[-1] * 1.2), 0, 1000)
    pdf = t.pdf(x_range, df, loc, scale)
    ax.plot(x_range, pdf, 'r-', linewidth=2, label=f'Fitted t-distribution (df={df:.2f})')
    
    # Shade extreme regions
    for i, level in enumerate(confidence_levels):
        extreme_value = theoretical_extremes[i]
        x_extreme = np.linspace(extreme_value * 1.2, extreme_value, 100)
        y_extreme = t.pdf(x_extreme, df, loc, scale)
        ax.fill_between(x_extreme, y_extreme, alpha=0.3, color=f'C{i+2}',
                       label=f'{level*100:.3f}% probability event: {extreme_value:.2%}')
    
    # Mark the historical minimum
    ax.axvline(x=historical_min, color='black', linestyle='--', 
              label=f'Historical Min: {historical_min:.2%}')
    
    # Add annotations
    for i, level in enumerate(confidence_levels):
        ax.annotate(f"{level*100:.3f}% event", 
                   xy=(theoretical_extremes[i], t.pdf(theoretical_extremes[i], df, loc, scale)),
                   xytext=(theoretical_extremes[i] * 0.8, t.pdf(theoretical_extremes[i], df, loc, scale) * 0.5),
                   arrowprops=dict(facecolor='black', shrink=0.05, width=1.5, headwidth=8),
                   fontsize=10, fontweight='bold')
    
    # Set titles and labels
    ax.set_title(f'Extended Tail Distribution for {ticker} Monthly Returns (Beyond Historical Data)', 
                fontsize=14, fontweight='bold')
    ax.set_xlabel('Monthly Return', fontsize=12)
    ax.set_ylabel('Density', fontsize=12)
    
    # Add explanatory text
    years_of_data = len(monthly_returns) // 12
    textstr = f"""
    Historical data: ~{years_of_data} years
    Observed minimum: {historical_min:.2%}
    T-distribution parameters: df={df:.2f}, loc={loc:.4f}, scale={scale:.4f}
    
    The extended tail shows theoretical extreme events that
    might not be present in limited historical data.
    """
