...
def _open_high_low_close(closes):
    """Build a tz-naive open_high_low_close frame with a Close column (the only one used)."""
...
def _open_high_low_close_with_calibration(prior_returns_pct, overnight_pct, start=28000.0):
    """open_high_low_close whose last return is the overnight move and prior 60 are calibration.
    ...
    return _open_high_low_close(closes), closes[-1]
...
class TestMorningBrief:
    ...
    def test_uses_default_repository_when_none_injected(self):
        ...
        with pytest.MonkeyPatch.context() as mp:
            mp.setattr(
                "fentu.explatoryservices.morning_brief.ReturnsRepository._raw_open_high_low_close",
                lambda self, t: hsi,
            )
            brief = morning_brief()
        assert brief == "HSI 23,000 (+4.55% o/n)"
...
class TestMorningBriefMadChute:
    ...
    def test_signed_mad_multiple_escalator_on_mild_up_move(self):
        ...
        hsi, last = _open_high_low_close_with_calibration(
